...
def scrape_all_pages(base_url, month, year, output_file):
    page_number = 1
    base_date_obj = datetime(year, month, 1)  # Now datetime refers to the class, not the module
    stop_scraping = False
    ...
    with open(output_file, mode='w', newline='', encoding='utf-8') as file:
        ...
        # Continue scraping until we hit the stopping condition
        while not stop_scraping:
            ...
            # If no actions are found, break out of the loop
            if not actions:
            ...
            for action in actions:
                ...
                # Stop if the date is before January 2023
                if date_object < base_date_obj:
                    stop_scraping = True
                    break
                
                # Write each action data directly to the CSV file
...
# Function to fetch agency names for each enforcement action link
def fetch_agency(link):
    try:
...
# Function to get agency names and update CSV with agency data
def add_agencies_to_csv(output_file):
    df = pd.read_csv(output_file)
    df['Agency'] = get_agencies(df)
    df.to_csv(output_file, index=False)

# Function to get agencies concurrently for CSV
def get_agencies(df):
    with ThreadPoolExecutor(max_workers=10) as executor:
...
# Scrape all pages and write directly to CSV
scrape_all_pages(base_url, 1, 2023, output_file)

# Read back CSV and add Agency column
