...
```{python}
#using my partner's function to subset data from 2021
#it is taking longer bc it has to go over several pages to find all the articles form 2021 onward!
from bs4 import BeautifulSoup
import requests
import pandas as pd
from datetime import datetime

def scrape_all_pages(base_url, month, year):
    page_number = 1
    stop_scraping = False
    data = []
    ...
    while not stop_scraping:
        ...
        for action in actions:
            ...
            date = pd.to_datetime(date_tag.get_text(strip=True)) if date_tag else None
            
            if date and date < datetime(year, month, 1):
                stop_scraping = True
                break
            ...
            # Fetching the agency information
            agency = 'N/A'  # Default if no agency info is found
            detail_response = requests.get(link)
            detail_soup = BeautifulSoup(detail_response.content, 'html.parser')
            agency_tag = detail_soup.find("ul", class_="usa-list usa-list--unstyled margin-y-2")
            if agency_tag:
                agency_details = agency_tag.find("span", class_="padding-right-2 text-base")
                agency = agency_details.next_sibling.strip() if agency_details and agency_details.next_sibling else 'N/A'
...
# Now call the function and filter from January 2021
base_url = 'https://oig.hhs.gov/fraud/enforcement/'
df_2021 = scrape_all_pages(base_url, 1, 2021)

# Filtering only for actions from January 2021 onwards
df_2021 = df_2021[df_2021['Date'] >= datetime(2021, 1, 1)]
print(df_2021.head())
...
df_2021['State'] = df_2021['Agency'].apply(lambda x: x.split("District of ")[-1] if "District of " in x else None)
print(df_2021.head['State'](5))
```
